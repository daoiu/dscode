...
from dscode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from dscode.commands.completion import CompletionPopup
from dscode.commands.handlers import register_all_commands
from dscode.config import MCPServerConfig, ProviderConfig
from dscode.hooks import HookContext, HookEngine, load_hooks
from dscode.conversation import ConversationManager, Message
from dscode.mcp import MCPManager
from dscode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from dscode.permissions import (
...
from dscode.agents.task_manager import TaskManager
from dscode.agents.trace import TraceManager
from dscode.agents.notification import inject_task_notifications
from dscode.commands.handlers.tasks import create_tasks_command
from dscode.skills.executor import SkillExecutor
from dscode.skills.loader import SkillLoader
from dscode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from dscode.cache import FileCache
from dscode.tools import ToolRegistry, create_default_registry
from dscode.tools.agent_tool import AgentTool
from dscode.tools.ask_user import AskUserEvent, AskUserTool
from dscode.tools.impl.tool_search import ToolSearchTool
from dscode.tools.load_skill import LoadSkill
from dscode.worktree.cleanup import start_stale_cleanup_task
from dscode.worktree.manager import WorktreeManager
from dscode.commands.handlers.worktree import create_worktree_command
from dscode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".dscode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".dscode" / "history"
...
class DSCodeApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_DSCODE_THEME)
        self.theme = "dscode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".dscode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".dscode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".dscode" / "permissions.local.yaml",
            ),
        ...
        from dscode.filehistory import FileHistory
        ...
        from dscode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from dscode.config import WorktreeConfig
        ...
        from dscode.tools.enter_worktree import EnterWorktreeTool
        from dscode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from dscode.teams.manager import TeamManager
        from dscode.tools.team_create import TeamCreateTool
        from dscode.tools.team_delete import TeamDeleteTool
        ...
        from dscode.commands.handlers.trace import create_trace_command
        ...
        from dscode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from dscode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from dscode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from dscode.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from dscode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from dscode.permission_dialog import InlinePermissionWidget
