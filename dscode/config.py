...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".dscode" / "config.yaml",
        cwd / ".dscode" / "config.yaml",
        cwd / ".dscode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .dscode/config.yaml "
            "in project or ~/.dscode/config.yaml"
        )
    return merged
