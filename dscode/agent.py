...
from dscode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from dscode.conversation import ThinkingBlock as ConvThinkingBlock
...
from dscode.hooks import HookContext, HookEngine, ToolRejectedError
from dscode.hooks.engine import HookNotification
from dscode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from dscode.tools import ToolRegistry
from dscode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".dscode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".dscode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from dscode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from dscode.context.manager import (
