...
from dscode.worktree.models import WorktreeSession
...
def _session_path(dscode_dir: Path) -> Path:
    return dscode_dir / SESSION_FILENAME
...
def load_worktree_session(dscode_dir: Path) -> WorktreeSession | None:
    path = _session_path(dscode_dir)
    if not path.exists():
        return None
    try:
