...
from dscode.commands.registry import Command, CommandContext, CommandType
...
async def handle_compact(ctx: CommandContext) -> None:
    ...
    from dscode.agent import CompactNotification, ErrorEvent
    ...
    if isinstance(result, CompactNotification):
        ...
        if ctx.session is not None and result.boundary is not None:
            from dscode.memory.session import make_compact_boundary
