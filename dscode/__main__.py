...
from dscode.config import ConfigError, load_config
from dscode.hooks import HookConfigError, HookEngine, load_hooks
from dscode.permissions import PermissionMode


def main() -> None:
    # 先确保 .dscode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".dscode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".dscode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="dscode", description="DSCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from dscode.app import DSCodeApp
    from dscode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    from dscode.agent import Agent
    from dscode.client import create_client, resolve_context_window
    from dscode.conversation import ConversationManager
    from dscode.memory.instructions import load_instructions
    ...
    from dscode.tools import create_default_registry
